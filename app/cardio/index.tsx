...
import { CardioEntry } from '../../src/types';
  ...
  return (
          ...
          {entriesHoje.map((entry) => (
            <View key={entry.id} style={styles.entryCard}>
              <View style={styles.entryHeader}>
                <Ionicons name={getTipoIcon(entry.tipo) as any} size={20} color={COR_CARDIO} />
                <Text style={styles.entryTipo}>{getTipoLabel(entry.tipo, entry.tipoOutro)}</Text>
                <TouchableOpacity onPress={() => confirmarDeletar(entry.id)}>
                  <Ionicons name="close-circle" size={20} color="#666" />
                </TouchableOpacity>
              </View>
              <View style={styles.entryStats}>
                <Text style={styles.entryStat}>⏱ {entry.duracaoMinutos}min</Text>
                <Text style={styles.entryStat}>🔥 {entry.calorias}kcal</Text>
                {entry.distanciaKm !== undefined && (
                  <Text style={styles.entryStat}>📏 {entry.distanciaKm}km</Text>
                )}
              </View>
              {entry.observacao && (
          ...
          <View style={styles.calCard}>
            ...
            {linhas.map((linha, li) => (
              <View key={li} style={styles.semana}>
                {linha.map((dia, di) => {
                  if (dia === null) return <View key={`e-${li}-${di}`} style={styles.diaCelula} />;
                  ...
                  return (
                    <TouchableOpacity
                      ...
                      onPress={() => { setSelectedDate(diaStr); setShowCalendar(false); }}
                      activeOpacity={0.6}
                    >
                      <View style={[styles.diaCircle, ehHoje && styles.diaCircleHoje]}>
                        <Text style={[styles.diaNumero, ehHoje && styles.diaNumeroHoje]}>{dia}</Text>
                      </View>
    ...
    >
      <View style={styles.overlay}>
        <TouchableOpacity style={styles.backdrop} activeOpacity={1} onPress={() => setSelectedDate('')} />
        <View style={styles.modal}>
          <TouchableOpacity style={styles.btnFechar} onPress={() => setSelectedDate('')}>
          ...
          <ScrollView contentContainerStyle={styles.modalConteudo} showsVerticalScrollIndicator={false}>
            ...
            <Text style={styles.modalData}>{formatarDataBR(selectedDate)}</Text>
            {entriesDoDia.length === 0 ? (
              <Text style={styles.semRegistro}>Nenhum cardio neste dia.</Text>
            ) : (
              <>
                <View style={styles.totalCard}>
                  <Ionicons name="checkmark-circle" size={20} color={COR_CARDIO} />
                  <Text style={styles.totalTexto}>
                    Total: {entriesDoDia.reduce((a, e) => a + e.duracaoMinutos, 0)}min • {entriesDoDia.reduce((a, e) => a + e.calorias, 0)}kcal
                  </Text>
                </View>
                {entriesDoDia.map((entry) => (
                  <View key={entry.id} style={styles.entryCard}>
                    <View style={styles.entryHeader}>
                      <Ionicons name={ICON_MAP[entry.tipo] as any} size={18} color={COR_CARDIO} />
                      <Text style={styles.entryTipo}>{getTipoLabel(entry.tipo, entry.tipoOutro)}</Text>
                    </View>
                    <View style={styles.entryStats}>
                      <Text style={styles.entryStat}>⏱ {entry.duracaoMinutos}min</Text>
...
const styles = StyleSheet.create({
  ...
  diaCelula: { flex: 1, alignItems: 'center', paddingVertical: 6, minHeight: 44 },
  diaCircle: { width: 32, height: 32, borderRadius: 16, alignItems: 'center', justifyContent: 'center' },
  diaCircleHoje: { backgroundColor: COR_PRIMARIA },
  diaNumero: { fontSize: 14, color: '#fff' },
  diaNumeroHoje: { fontWeight: 'bold' },
  bolinha: { width: 5, height: 5, borderRadius: 3, backgroundColor: COR_CARDIO, marginTop: 2 },
  ...
  modalData: { fontSize: 18, fontWeight: 'bold', color: '#fff', marginBottom: 20, textAlign: 'center' },
  semRegistro: { fontSize: 14, color: '#888', textAlign: 'center', paddingVertical: 20 },
  totalCard: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 8,
